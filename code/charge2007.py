# -*- coding: utf-8 -*-
"""
Created on Tue Nov 15 20:12:22 2022
...
def pocket_coordinate(name,P_atom,L_atom,cutoff):
    ...
    '''
    ########################################################################################
    t1 = '../data/2007/pqr/' + name + '_pocket.pqr'
    f1 = open(t1,'r')
    p_list = []
    ...
    t2 = '../data/2007/refined-set/' + name + '/' + name + '_ligand.mol2'
    f2 = open(t2,'r')
    l_list = []
    ...
    filename = '../data/2007/charge_atom_combination'  + name + '/' + 'Protein_' + P_atom + '_' + 'Ligand_' + L_atom + '_coordinate.csv'
...
def write_pocket_coordinate_to_file(start,end,cutoff):
    ...
    for i in range(start,end):
        name = all_data[i]
        os.mkdir('../data/2007/charge_atom_combination' + name) 
...
def get_feature(name,max_filtration,step,k):
    ...
    feature_file1 = '../data/2007/charge_eigenvalue/' + name + '_hop1_eigenvalue.txt'
    f = open(feature_file1,'w')
    f.writelines(str(vertexmatrix1))
    f.close()
    feature_file2 = '../data/2007/charge_eigenvalue/' + name + '_hop2_eigenvalue.txt'
    f = open(feature_file2,'w')
    f.writelines(str(vertexmatrix2))
    f.close()
    feature_file3 = '../data/2007/charge_eigenvalue/' + name + '_hop3_eigenvalue.txt'
...
def get_eigenvalue_median_and_mean(name,hop):
    ...
    nonzero_file = '../data/2007/charge_eigenvalue/' + name + '_hop' + str(hop) + '.txt'
    f = open(nonzero_file)
    ...
    file1 = '../data/2007/charge_eigenvalue/' + name + '_hop' + str(hop) + '_median.csv'
    np.savetxt(file1 , matrix1 , delimiter = ',')        
    file2 = '../data/2007/charge_eigenvalue/' + name + '_hop' + str(hop) + '_mean.csv'
...
def pocket_test_feature(hop,typ):
    ...
    for name in test_data:
        feature_file = '../data/2007/charge_eigenvalue/' + name + '_hop'+ str(hop) +'_'+ typ +'.csv'
        feature = np.loadtxt(feature_file,delimiter = ',')
        matrix.append(feature.tolist())
    feature_file = '../data/2007/charge_feature/test_hop' + str(hop) + '_' + typ +'.csv'
...
def pocket_train_feature(hop,typ):
    ...
    for name in train_data:
        ...
        feature = np.loadtxt(feature_file,delimiter = ',')
        matrix.append(feature.tolist())
    feature_file = '../data/2007/charge_feature/train_hop' + str(hop) +'_'+ typ  +'.csv'
    np.savetxt(feature_file , np.array(matrix) , delimiter = ',')
    print(np.array(matrix).shape)
...
def get_combined_feature(typ):
    ...
    filename = '../data/2007/charge_feature/' + typ + 'hop123_mm.csv'
...
def get_target_matrix_of_train():
    ...
    t1 = '../data/2007/refined-set/INDEX.2007.refined.data'
    f1 = open(t1,'r')
    ...
    target_file = '../data/2007/charge_feature/train_target.csv'
...
def get_target_matrix_of_test():
    ...
    target_file = '../data/2007/charge_feature/test_target.csv'
    np.savetxt(target_file , target_matrix , delimiter = ',')  
...
def get_pearson_correlation(hop,typ):
    feature_matrix_of_train = np.loadtxt( '../data/2007/charge_feature/train_hop'+ str(hop) +'_mm.csv',delimiter=',' )
    target_matrix_of_train = np.loadtxt( '../data/2007/charge_feature/train_target.csv',delimiter=',' )
    feature_matrix_of_test = np.loadtxt( '../data/2007/charge_feature/test_hop'+ str(hop) +'_mm.csv',delimiter=',' )
    target_matrix_of_test = np.loadtxt( '../data/2007/charge_feature/test_target.csv',delimiter=',' )
    number = 10
    P = np.zeros((number,1))
    M = np.zeros((number,1))
    ...
    print('for data 2007 , 10 results for charge distance-model are:')
    print(P)
...
def run_PDBbind_2007():
    ###############################################################################
    '''
    run this function, you can get the results for data2007 
