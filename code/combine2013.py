...
def combine_feature_of_euclid_and_charge(typ,hop):
    file1 = '../data/2013/euclid_feature/' + typ + '_mm_hop' + str(hop) + '.csv'
    file2 = '../data/2013/charge_feature/' + typ + '_mm_hop' + str(hop) + '.csv'
    feature1 = np.loadtxt(file1,delimiter = ',')
    feature2 = np.loadtxt(file2,delimiter = ',')
    
    com_feature = np.hstack((feature1,feature2))
    filename = '../data/2013/combine_euclid_and_charge_feature/' + typ + '_mm_hop' + str(hop) + '.csv'
...
def get_pearson_correlation(hop):
    feature_matrix_of_train = np.loadtxt( '../data/2013/combine_euclid_and_charge_feature/train_hop'+ str(hop) +'_mm.csv',delimiter=',' )
    target_matrix_of_train = np.loadtxt( '../data/2013/charge_feature/train_target.csv',delimiter=',' )
    feature_matrix_of_test = np.loadtxt( '../data/2013/combine_euclid_and_charge_feature/test_hop'+ str(hop) +'_mm.csv',delimiter=',' )
    ...
    print('for data 2013 , 10 results for euclid and charge distance-model are:')
    print(P)
